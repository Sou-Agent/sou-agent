...
class SessionStore:
    ...
    def get_or_create_session(
        ...
        if self._db and db_end_session_id:
            ...
            # Training data: capture the session that was just ended (before new one is created)
            try:
                from training.collector import capture_session, is_enabled
                if is_enabled():
                    _old_session_row = self._db.get_session(db_end_session_id)
                    _platform_str = (
                        _old_session_row.get("source", "")
                        if _old_session_row
                        else (source.platform.value if hasattr(source.platform, "value") else str(source.platform))
                    )
                    capture_session(
                        ...
                        extra_meta={
                            "completed": False,
                            "exit_reason": auto_reset_reason or "timeout",
                        },
                    )
            except Exception:
                pass
