...
def build_injected_prompt(snapshot: Dict[str, Any], decision: Dict[str, Any],
    ...
    for s in decision.get("signals", []):
        ...
        if action == "respond":
            lines += [
                ...
                "",
                "       GROUND FIRST: Use session_search or mnemosyne_recall to check",
                "       if you've already discussed this topic before. Don't re-ask",
                "       questions you've already had answered.",
                "",
    ...
    lines += [
        "",
        "--- CONTINUITY: RECALL YESTERDAY'S MESSAGES ---",
        "Before acting on any signal, ground yourself in recent history.",
        "This is NOT a fresh start — you've had conversations before today.",
        "",
        "Use these tools to recall context:",
        "  mnemosyne_recall(query=...)  — search durable memories (preferences, people, facts)",
        "  session_search(query=...)     — search past conversation transcripts by topic",
        "  session_search()              — browse recent sessions (no query = chronological)",
        "",
        "Recommended: start with session_search() to see what's happened recently,",
        "then mnemosyne_recall() for specific people or topics. This prevents you",
        "from asking questions you've already answered or acting on stale context.",
        "",
    ]
