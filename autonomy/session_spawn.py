...
def build_injected_prompt(snapshot: Dict[str, Any], decision: Dict[str, Any],
    ...
    lines += [
        ...
        "                 Create an intent if there's something to follow up on later.",
        "                 You'll come back to this on the next cycle.",
        "  self_reflect — You MUST actually use journal tools. This is NOT optional.",
        "                 Call journal_write for new entries, journal_append to existing",
        "                 entries, journal_remove to clean up stale ones, journal_read to",
        "                 re-engage. Create an intent (origin='self_reflect') if a follow-up",
        "                 emerges during reflection.",
